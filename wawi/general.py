...
def get_coherency_matrix(S):
    """ 
    Compute the coherency matrix from a cross-spectral density (CSD) matrix.

    Parameters
    ----------
    S : ndarray
        A 3D array representing the cross-spectral density matrix,
        with dimensions (N_dofs, N_dofs, N_frequencies).

    Returns
    -------
    coh : ndarray
        A 3D array representing the coherency matrix, with the same
        dimensions as the input `S`. Each element `coh[i,j,k]` is
        calculated as `S[i,j,k] / sqrt(S[i,i,k] * S[j,j,k])`.

    Notes
    -----
    The coherency matrix is a normalized version of the CSD matrix,
    where each element represents the correlation between two signals
    at a given frequency. It is typically a complex-valued matrix.
    ...
    for i in range(S.shape[0]):
        for j in range(S.shape[1]):
            if i == j:
                coh[i,j] = coh[i,j]*0 + 1.0
            else:
                coh[i,j] = S[i,j]/(np.sqrt(S[i,i]*S[j,j]))
